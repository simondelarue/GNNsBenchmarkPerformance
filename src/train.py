...
class Trainer:
    ...
    def train_eval(self, model, dataset, penalized):
        ...
        for _ in range(3):
            alg = get_model(model)
            labels_pred = alg.fit_predict(dataset, self.train_idx)
            train_acc += alg.accuracy(dataset, labels_pred, self.train_idx, penalized)
            test_acc += alg.accuracy(dataset, labels_pred, self.test_idx, penalized)
